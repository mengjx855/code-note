...
# modified date: 2024-02-07, 00:14:22
...
if [[ $fq =~ "," ]];then
    ...
    spades.py --meta -1 $fq1 -2 $fq2 -k $klist -o $out -t $trds > /dev/null 2>/dev/null || { rm -r $out && exit 1; }
else
    spades.py --meta -s $fq -k $klist -o $out -t $trds > /dev/null 2>/dev/null || { rm -r $out && exit 1; }
fi
