...
\e[1;31m Configs format (tab-delimited file): \e[0m
 ...
 Binning (netabat2) multiple coverage mode was performed between s1, s2, s3, s4."
    exit 127
fi
...
if [ ! -d ${out}_cls ];then mkdir ${out}_cls;fi
...
for i in ${!ctg[*]};do
    ...
    parallel -j ${jobs} --colsep="\t" /usr/bin/time -v -a -o ${tmp}/time.log \
        ...
        bzip2 ${tmp}/{1}.depth \&\& \
        mv ${out}_fna/{1}.bin ${out}_cls/{1}.bin.cls \&\& \
        mv ${out}_fna/{1}.bin.unbinned.fa ${out}_fna/{1}.bin.lowDepth.fa ${out}_fna/{1}.bin.tooShort.fa ${out}_unbinned \&\& \
